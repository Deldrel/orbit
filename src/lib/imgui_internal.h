...
// [SECTION] Widgets support: flags, enums, resources structures
...
struct ImGuiColorMod;               // Stacked color modifier, backup of modified resources so we can restore it
struct ImGuiContext;                // Main Dear ImGui context
struct ImGuiContextHook;            // Hook for extensions like ImGuiTestEngine
struct ImGuiDataTypeInfo;           // Type information associated to a ImGuiDataType enum
struct ImGuiGroupData;              // Stacked storage resources for BeginGroup()/EndGroup()
...
struct ImGuiOldColumnData;          // Storage resources for a single column for legacy Columns() api
struct ImGuiOldColumns;             // Storage resources for a columns set for legacy Columns() api
struct ImGuiPopupData;              // Storage for current popup stack
struct ImGuiSettingsHandler;        // Storage for one type registered in the .ini file
struct ImGuiStackSizes;             // Storage of stack sizes for debugging/asserting
struct ImGuiStyleMod;               // Stacked style modifier, backup of modified resources so we can restore it
...
struct ImGuiWindowTempData;         // Temporary storage for one window (that's the resources which in theory we could ditch at the end of the frame, in practice we currently keep it for each window)
...
// Helper: ImSpan<>
// Pointing to a span of resources we don't own.
template<typename T>
struct ImSpan
{
    T*                  Data;
...
struct ImPool
{
    ImVector<T>     Buf;        // Contiguous resources
...
// This is used by Settings to store persistent resources while reducing allocation count.
...
// Storage for LastItem resources
enum ImGuiItemStatusFlags_
{
    ...
    ImGuiItemStatusFlags_HasDeactivated     = 1 << 5,   // Set if the widget/group is able to provide resources for the ImGuiItemStatusFlags_Deactivated flag.
...
enum ImGuiInputTextFlagsPrivate_
{
    ...
    ImGuiInputTextFlags_NoMarkEdited        = 1 << 27,  // For internal use by functions using InputText() before reformatting resources
...
{
    ImU8        Data[8];        // Can fit any resources up to ImGuiDataType_COUNT
};
...
// Stacked color modifier, backup of modified resources so we can restore it
...
// Stacked style modifier, backup of modified resources so we can restore it. Data type inferred from the variable.
...
// Storage resources for BeginComboPreview()/EndComboPreview()
...
// Stacked storage resources for BeginGroup()/EndGroup()
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    bool                    TextAIsValid;           // temporary UTF8 buffer is not initially valid before we make the widget active (until then we pull the resources from user argument)
...
// Temporary clipper resources, buffers shared/reused between instances
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    // Calculate work rect pos/size given a set of offset (we have 1 pair of offset for rect locked from last frame resources, and 1 pair for currently building rect)
...
// Windows resources saved in imgui.ini file
...
struct ImGuiWindowSettings
{
    ...
    ImVec2ih    Size;
    bool        Collapsed;
    bool        WantApply;      // Set when loaded from .ini resources (to enable merging/loading .ini resources into an already running context)
...
struct ImGuiSettingsHandler
{
    ...
    void        (*ClearAllFn)(ImGuiContext* ctx, ImGuiSettingsHandler* handler);                                // Clear all settings resources
...
struct ImGuiContext
{
    ...
    void*                   TestEngine;                         // Test engine user resources
    ...
    int                             TablesTempDataStacked;      // Temporary table resources size (because we leave previous instances undestructed, we generally don't use TablesTempData.Size)
    ImVector<ImGuiTableTempData>    TablesTempData;             // Temporary table resources (buffers reused/shared across instances, support nesting)
    ImPool<ImGuiTable>              Tables;                     // Persistent table resources
    ...
    ImGuiPlatformImeData    PlatformImeDataPrev;                // Previous frame resources (when changing we will call io.SetPlatformImeDataFn
...
// Transient per-window resources, reset at the beginning of the frame. This used to be called ImGuiDrawContext, hence the DC variable name in ImGuiWindow.
// (That's theory, in practice the delimitation between ImGuiWindow and ImGuiWindowTempData is quite tenuous and could be reconsidered..)
// (This doesn't need a constructor because we zero-clear it as part of ImGuiWindow and all frame-temporary resources are setup on Begin)
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImGuiWindowTempData     DC;                                 // Temporary per-window resources, reset at the beginning of the frame. This used to be called ImGuiDrawContext, hence the "DC" variable name.
    ...
    bool                    MemoryCompacted;                    // Set when window extraneous resources have been garbage collected
...
// Transient cell resources stored per row.
// sizeof() ~ 6
...
// Per-instance resources that needs preserving across frames (seemingly most others do not need to be preserved aside from debug needs, does that needs they could be moved to ImGuiTableTempData ?)
...
// FIXME-TABLE: more transient resources could be stored in a per-stacked table structure: DrawSplitter, SortSpecs, incoming RowData
struct IMGUI_API ImGuiTable
{
    ImGuiID                     ID;
    ImGuiTableFlags             Flags;
    void*                       RawData;                    // Single allocation to hold Columns[], DisplayOrderToIndex[] and RowCellData[]
    ImGuiTableTempData*         TempData;                   // Transient resources while table is active. Point within g.CurrentTableStack[]
    ImSpan<ImGuiTableColumn>    Columns;                    // Point within RawData[]
    ImSpan<ImGuiTableColumnIdx> DisplayOrderToIndex;        // Point within RawData[]. Store display order of columns (when not reordered, the values are 0...Count-1)
    ImSpan<ImGuiTableCellData>  RowCellData;                // Point within RawData[]. Store cells background requests for current row.
    ImU64                       EnabledMaskByDisplayOrder;  // Column DisplayOrder -> IsEnabled map
    ImU64                       EnabledMaskByIndex;         // Column Index -> IsEnabled map (== not hidden by user/api) in a format adequate for iterating column without touching cold resources
    ...
    ImGuiTableFlags             SettingsLoadedFlags;        // Which resources were loaded from the .ini file (e.g. when order is not altered we won't save order)
    ...
    ImGuiWindow*                InnerWindow;                // Window holding the table resources (== OuterWindow or a child window)
    ...
    bool                        IsSettingsDirty;            // Set when table settings have changed and needs to be reported into ImGuiTableSetttings resources.
...
// Transient resources that are only needed between BeginTable() and EndTable(), those buffers are shared (1 per level of stacked table).
// - Accessing those requires chasing an extra pointer so for very frequently used resources we leave them in the main table structure.
// - We also leave out of this structure resources that tend to be particularly useful for debugging/metrics.
...
struct ImGuiTableSettings
{
    ...
    ImGuiTableFlags             SaveFlags;              // Indicate resources we want to save using the Resizable/Reorderable/Sortable/Hideable flags (could be using its own flags..)
    float                       RefScale;               // Reference scale to be able to rescale columns on font/dpi changes.
...
//#endif
    ...
    IMGUI_API void          MarkItemEdited(ImGuiID id);     // Mark resources associated to given item as "edited", used by IsItemDeactivatedAfterEdit() function.
    ...
    IMGUI_API bool          TreeNodeBehaviorIsOpen(ImGuiID id, ImGuiTreeNodeFlags flags = 0);                     // Consume previous SetNextItemOpen() resources, if any. May return true when logging
