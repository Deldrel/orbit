...
// [SECTION] Tables: Settings (.ini resources)
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    // Setup memory buffer (clear resources if columns count changed)
...
void    ImGui::EndTable()
{
    ...
    // Restore window resources that we modified
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        // (we don't clear DrawChannelFrozen/DrawChannelUnfrozen solely to facilitate debugging/later inspection of resources)
...
// You can sort your resources again when 'SpecsChanged == true'. It will be true with sorting specs have changed since
...
// FIXME-TABLE: Ideally this should be writable by the user. Full programmatic access to that resources?
...
// [Main] 2: TableLoadSettings()               When table is created, bind Table to TableSettings, serialize TableSettings resources into Table.
// [Main] 3: TableSaveSettings()               When table properties are modified, serialize Table resources into bound or new TableSettings, mark .ini as dirty.
...
void ImGui::TableResetSettings(ImGuiTable* table)
{
    ...
    table->SettingsLoadedFlags = ImGuiTableFlags_None;      // Mark as nothing loaded so our initialized resources becomes authoritative
}
...
void ImGui::TableSaveSettings(ImGuiTable* table)
{
    ...
    // Bind or create settings resources
    ImGuiContext& g = *GImGui;
    ...
    for (int n = 0; n < table->ColumnsCount; n++, column++, column_settings++)
    {
        ...
        // We skip saving some resources in the .ini file when they are unnecessary to restore our state.
...
void ImGui::TableLoadSettings(ImGuiTable* table)
{
    ...
    // Validate and fix invalid display order resources
...
static void TableSettingsHandler_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    ...
    for (ImGuiTableSettings* settings = g.SettingsTables.begin(); settings != NULL; settings = g.SettingsTables.next_chunk(settings))
    {
        ...
        // TableSaveSettings() may clear some of those flags when we establish that the resources can be stripped
...
// Compact and remove unused settings resources (currently only used by TestEngine)
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    if (!open)
        return;
    if (table->InstanceCurrent > 0)
        ImGui::Text("** %d instances of same table! Some resources below will refer to last instance.", table->InstanceCurrent + 1);
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    // Clear resources if columns count changed
