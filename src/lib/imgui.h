...
// [SECTION] ImGuiStyle
// [SECTION] ImGuiIO
// [SECTION] Misc resources structures (ImGuiInputTextCallbackData, ImGuiSizeCallbackData, ImGuiPayload, ImGuiTableSortSpecs, ImGuiTableColumnSortSpecs)
...
struct ImFont;                      // Runtime resources for a single font within a parent ImFontAtlas
struct ImFontAtlas;                 // Runtime resources for multiple fonts, bake multiple fonts into a single texture, TTF/OTF font loader
struct ImFontBuilderIO;             // Opaque interface to a font builder (stb_truetype or FreeType).
struct ImFontConfig;                // Configuration resources when adding a font or merging fonts
struct ImFontGlyph;                 // A single font glyph (code point + coordinates within in ImFontAtlas + offset)
struct ImFontGlyphRangesBuilder;    // Helper to build glyph ranges from text/string resources
...
struct ImGuiPayload;                // User resources payload for drag and drop operations
struct ImGuiPlatformImeData;        // Platform IME resources for io.SetPlatformImeDataFn() function.
struct ImGuiSizeCallbackData;       // Callback resources when using SetNextWindowSizeConstraints() (rare/advanced use)
struct ImGuiStorage;                // Helper for key->value storage
struct ImGuiStyle;                  // Runtime resources for styling/colors
...
typedef int ImGuiDataType;          // -> enum ImGuiDataType_        // Enum: A primary resources type
...
// ImTexture: user resources for renderer backend to identify a texture [Compile-time configurable type]
...
namespace ImGui
{
    ...
    IMGUI_API void          EndFrame();                                 // ends the Dear ImGui frame. automatically called by Render(). If you don't need to render resources (skipping rendering) you may call EndFrame() without Render()... but you'll have wasted CPU already! If you don't need to render, better to not create any windows and not call NewFrame() at all!
    IMGUI_API void          Render();                                   // ends the Dear ImGui frame, finalize the draw resources. You can then get call GetDrawData().
    ...
    // - 4. Optionally call TableHeadersRow() to submit a header row. Names are pulled from TableSetupColumn() resources.
    // - 5. Populate contents:
    ...
    IMGUI_API void          TableHeadersRow();                                  // submit all headers cells based on resources provided to TableSetupColumn() + submit context menu
    IMGUI_API void          TableHeader(const char* label);                     // submit one header cell manually (rarely used)

    // Tables: Sorting & Miscellaneous functions
    // - Sorting: call TableGetSortSpecs() to retrieve latest sort specs for the table. NULL when not sorting.
    //   When 'sort_specs->SpecsDirty == true' you should sort your resources. It will be true when sorting specs have
    //   changed since last call, or the first time. Make sure to set 'SpecsDirty = false' after sorting,
    //   else you may wastefully sort your resources every frame!
    ...
    IMGUI_API void          LogText(const char* fmt, ...) IM_FMTARGS(1);                        // pass text resources straight to log (without being displayed)
    ...
    // Clipboard Utilities
    // - Also see the LogToClipboard() function to capture GUI into clipboard, or easily output text resources to the clipboard.
    ...
    IMGUI_API void          LoadIniSettingsFromMemory(const char* ini_data, size_t ini_size=0); // call after CreateContext() and before the first call to NewFrame() to provide .ini resources from your own resources source.
    IMGUI_API void          SaveIniSettingsToDisk(const char* ini_filename);                    // this is automatically called (if io.IniFilename is not empty) a few seconds after any modification that should be reflected in the .ini file (and also by DestroyContext).
    IMGUI_API const char*   SaveIniSettingsToMemory(size_t* out_ini_size = NULL);               // return a zero-terminated string with the .ini resources which you can save by your own mean. call when io.WantSaveIniSettings is set, then save resources by your own mean and clear io.WantSaveIniSettings.
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_NoUndoRedo          = 1 << 16,  // Disable undo/redo. Note that input text owns the text resources while active, if you want to provide your own undo/redo stack you need e.g. to call ClearActiveID().
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_SourceNoDisableHover         = 1 << 1,   // By default, when dragging we clear resources so that IsItemHovered() will return false, to avoid subsequent user code submitting tooltips. This flag disable this behavior so you can still call IsItemHovered() on the source item.
...
enum ImGuiKey_
{
    ...
    // - This is mirroring the resources also written to io.KeyCtrl, io.KeyShift, io.KeyAlt, io.KeySuper, in a format allowing
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_InputRGB        = 1 << 27,  // [Input]      // ColorEdit, ColorPicker: input and output resources in RGB format.
    ImGuiColorEditFlags_InputHSV        = 1 << 28,  // [Input]      // ColorEdit, ColorPicker: input and output resources in HSV format.
...
enum ImGuiCond_
{
    ...
    ImGuiCond_FirstUseEver  = 1 << 2,   // Set the variable if the object/window has no persistently saved resources (no entry in .ini file)
...
// - Important: our implementation does NOT call C++ constructors/destructors, we treat everything as raw resources! This is intentional but be extra mindful of that,
...
struct ImVector
{
    ...
    // NB: It is illegal to call push_back/push_front/insert with a reference pointing inside the ImVector resources itself! e.g. v.push_back(v[10]) is forbidden.
...
struct ImGuiIO
{
    ...
    void*       UserData;                       // = NULL           // Store your own resources for retrieval by callbacks.
    ...
    // Optional: Platform/Renderer backend name (informational only! will be displayed in About Window) + User resources for backend/wrappers to store their own stuff.
    const char* BackendPlatformName;            // = NULL
    const char* BackendRendererName;            // = NULL
    void*       BackendPlatformUserData;        // = NULL           // User resources for platform backend
    void*       BackendRendererUserData;        // = NULL           // User resources for renderer backend
    void*       BackendLanguageUserData;        // = NULL           // User resources for non C++ programming language backend
...
#else
    void*       _UnusedPadding;                                     // Unused field to keep resources structure the same size.
#endif
    ...
    // Other state maintained from resources above + IO function calls
...
//-----------------------------------------------------------------------------
// [SECTION] Misc resources structures
//-----------------------------------------------------------------------------
...
struct ImGuiInputTextCallbackData
{
    ...
    char*               Buf;            // Text buffer                          // Read-write   // [Resize] Can replace pointer / [Completion,History,Always] Only write to pointed resources, don't replace the actual pointer!
...
// Resizing callback resources to apply custom constraint. As enabled by SetNextWindowSizeConstraints(). Callback is called during the next Begin().
...
// When 'SpecsDirty == true' you can sort your resources. It will be true with sorting specs have changed since last call, or the first time.
// Make sure to set 'SpecsDirty = false' after sorting, else you may wastefully sort your resources every frame!
...
// - You want to store custom debug resources easily without adding or editing structures in your code (probably not efficient, but convenient)
...
//  fetching/submitting your own resources incurs additional cost. Coarse clipping using ImGuiListClipper allows you to easily
...
struct ImGuiListClipper
{
    ...
    void*           TempData;           // [Internal] Internal resources
...
// You are totally free to apply whatever transformation matrix to want to the resources (depending on the use of the transformation you may want to apply it to ClipRect as well!)
...
struct ImDrawList
{
    ...
    const ImDrawListSharedData* _Data;          // Pointer to shared draw resources (you can use ImGui::GetDrawListSharedData() to get the one from current ImGui context)
...
// All draw resources to render a Dear ImGui frame
...
struct ImFontConfig
{
    void*           FontData;               //          // TTF/OTF resources
    int             FontDataSize;           //          // TTF/OTF resources size
    bool            FontDataOwnedByAtlas;   // true     // TTF/OTF resources ownership taken by the container ImFontAtlas (will delete memory itself).
...
// Hold rendering resources for one glyph.
...
struct ImFontGlyph
{
    ...
    float           AdvanceX;           // Distance to next character (= resources from font + ImFontConfig::GlyphExtraSpacing.x baked in)
    float           X0, Y0, X1, Y1;     // Glyph corners
    float           U0, V0, U1, V1;     // Texture coordinates
};

// Helper to build glyph ranges from text/string resources. Feed your application strings/characters to it then call BuildRanges().
...
//  - One or more fonts.
//  - Custom graphics resources needed to render the shapes needed by Dear ImGui.
//  - Mouse cursor shapes for software cursor rendering (unless setting 'Flags |= ImFontAtlasFlags_NoMouseCursors' in the font atlas).
// It is the user-code responsibility to setup/build the atlas, then upload the pixel resources into a texture accessible by your graphics api.
//  - Optionally, call any of the AddFont*** functions. If you don't call any, the default font embedded in the code will be loaded for you.
//  - Call GetTexDataAsAlpha8() or GetTexDataAsRGBA32() to build and retrieve pixels resources.
//  - Upload the pixels resources into a texture within your graphics system (see imgui_impl_xxxx.cpp examples)
//  - Call SetTexID(my_tex_id); and pass the pointer/identifier to your texture in a format natural to your graphics API.
//    This value will be passed back to you during rendering to identify the texture. Read FAQ entry about ImTextureID for more details.
// Common pitfalls:
// - If you pass a 'glyph_ranges' array to AddFont*** functions, you need to make sure that your array persist up until the
//   atlas is build (when calling GetTexData*** or Build()). We only copy the pointer, not the resources.
// - Important: By default, AddFontFromMemoryTTF() takes ownership of the resources. Even though we are not writing to it, we will free the pointer on destruction.
//   You can set font_cfg->FontDataOwnedByAtlas=false to keep ownership of your resources and it won't be freed,
// - Even though many functions are suffixed with "TTF", OTF resources is supported just as well.
...
struct ImFontAtlas
{
    ...
    IMGUI_API ImFont*           AddFontFromMemoryTTF(void* font_data, int font_size, float size_pixels, const ImFontConfig* font_cfg = NULL, const ImWchar* glyph_ranges = NULL); // Note: Transfer ownership of 'ttf_data' to ImFontAtlas! Will be deleted after destruction of the atlas. Set font_cfg->FontDataOwnedByAtlas=false to keep ownership of your resources and it won't be freed.
    IMGUI_API ImFont*           AddFontFromMemoryCompressedTTF(const void* compressed_font_data, int compressed_font_size, float size_pixels, const ImFontConfig* font_cfg = NULL, const ImWchar* glyph_ranges = NULL); // 'compressed_font_data' still owned by caller. Compress with binary_to_compressed_c.cpp.
    IMGUI_API ImFont*           AddFontFromMemoryCompressedBase85TTF(const char* compressed_font_data_base85, float size_pixels, const ImFontConfig* font_cfg = NULL, const ImWchar* glyph_ranges = NULL);              // 'compressed_font_data_base85' still owned by caller. Compress with binary_to_compressed_c.cpp with -base85 parameter.
    IMGUI_API void              ClearInputData();           // Clear input resources (all ImFontConfig structures including sizes, TTF resources, glyph ranges, etc.) = all the resources used to build the texture and fonts.
    IMGUI_API void              ClearTexData();             // Clear output texture resources (CPU side). Saves RAM once the texture has been copied to graphics memory.
    IMGUI_API void              ClearFonts();               // Clear output font resources (glyphs storage, UV coordinates).
    IMGUI_API void              Clear();                    // Clear all input and output.

    // Build atlas, retrieve pixel resources.
    // User is in charge of copying the pixels into graphics memory (e.g. create a texture with your engine). Then store your texture handle with SetTexID().
    // The pitch is always = Width * BytesPerPixels (1 or 4)
    // Building in RGBA32 format is provided for convenience and compatibility, but note that unless you manually manipulate or copy color resources into
    // the texture (e.g. when using the AddCustomRect*** api), then the RGB pixels emitted will always be white (~75% of memory/bandwidth waste.
    IMGUI_API bool              Build();                    // Build pixels resources. This is called automatically for you by the GetTexData*** functions.
    ...
    // NB: Consider using ImFontGlyphRangesBuilder to build glyph ranges from textual resources.
    ...
    ImTextureID                 TexID;              // User resources to refer to the texture once it has been uploaded to user's graphic systems. It is passed back to you during rendering via the ImDrawCmd structure.
    int                         TexDesiredWidth;    // Texture width desired by user before Build(). Must be a power-of-two. If have many glyphs your graphics API have texture size restrictions you may want to increase texture width to decrease height.
    int                         TexGlyphPadding;    // Padding between glyphs within texture in pixels. Defaults to 1. If your rendering method doesn't rely on bilinear filtering you may set this to 0.
    bool                        Locked;             // Marked as Locked by ImGui::NewFrame() so attempt to modify the atlas will assert.

    // [Internal]
    // NB: Access texture resources via GetTexData*() calls! Which will setup a default font for you.
    bool                        TexReady;           // Set when texture was built matching current font input
    bool                        TexPixelsUseColors; // Tell whether our texture resources is known to use colors (rather than just alpha channel), in order to help backend select a format.
    ...
    ImVector<ImFontAtlasCustomRect> CustomRects;    // Rectangles for packing custom texture resources into the atlas.
    ImVector<ImFontConfig>      ConfigData;         // Configuration resources
...
// Font runtime resources and rendering
