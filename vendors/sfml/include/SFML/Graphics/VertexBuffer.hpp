...
public:
    ...
    ///
    /// If resources is going to be updated once or more every frame,
    /// set the usage to Stream. If resources is going to be set once
    ...
    enum Usage
    {
        Stream,  ///< Constantly changing resources
        Dynamic, ///< Occasionally changing resources
    ...
    /// containing the vertex resources.
    ///
    ...
    /// going to be used in terms of resources update frequency.
    ///
    /// After changing the usage specifier, the vertex buffer has
    /// to be updated with new resources for the usage specifier to
    /// take effect.
    ///
...
///
/// Unlike sf::VertexArray, the vertex resources is stored in
/// graphics memory.
///
/// In situations where a large amount of vertex resources would
/// have to be transferred from system memory to graphics memory
/// every frame, using sf::VertexBuffer can help. By using a
/// sf::VertexBuffer, resources that has not been changed between frames
/// does not have to be re-transferred from system to graphics
/// memory as would be the case with sf::VertexArray. If resources transfer
...
/// the application. This allows the user to take full control of resources
...
/// vertex resources in multiple distinct regions of the buffer simultaneously.
...
/// across multiple threads as long as there are no other resources dependencies.
///
...
/// pending resources transfers complete before the vertex buffer is sourced
