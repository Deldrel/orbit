...
public:
    ...
    /// \param sizeInBytes Size of the resources to load, in bytes
    ///
    /// \return True if loading succeeded, false if it failed
    ///
...
/// \ingroup audio
///
/// A sound buffer holds the resources of a sound, which is
...
///
/// Sound buffers alone are not very useful: they hold the audio resources
...
/// side, a sf::Sound is a lightweight object, which can use the audio resources
/// of a sound buffer and change the way it is played without actually
/// modifying that resources. Note that it is also possible to bind
