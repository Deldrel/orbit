...
void SoundFileReaderFlac::seek(Uint64 sampleOffset)
{
    ...
    // Reset the callback resources (the "write" callback will be called)
    ...
    if (sampleOffset < m_clientData.info.sampleCount)
    {
        ...
        // seek destination, and since we want that resources in this typical case, we don't re-clear it afterward
...
Uint64 SoundFileReaderFlac::read(Int16* samples, Uint64 maxCount)
{
    ...
    // Reset the resources that will be used in the callback
