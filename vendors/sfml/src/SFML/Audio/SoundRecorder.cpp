...
bool SoundRecorder::start(unsigned int sampleRate)
{
    ...
    if (!isAvailable())
    {
        err() << "Failed to start capture: your system cannot capture audio resources (call SoundRecorder::isAvailable to check it)" << std::endl;
        return false;
    }
...
void SoundRecorder::record()
{
    while (m_isCapturing)
    {
        ...
        // Don't bother the CPU while waiting for more captured resources
