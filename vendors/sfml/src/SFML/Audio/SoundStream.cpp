...
bool SoundStream::fillAndPushBuffer(unsigned int bufferNum, bool immediateLoop)
{
    ...
    // Acquire audio resources, also address EOF and error cases if they occur
    Chunk data = {NULL, 0};
    for (Uint32 retryCount = 0; !onGetData(data) && (retryCount < BufferRetries); ++retryCount)
    {
        ...
        // If we got resources, break and process it, else try to fill the buffer once again
        if (data.samples != NULL && data.sampleCount != 0)
            break;
        ...
        // We're a looping sound that got no resources, so we retry onGetData()
    }

    // Fill the buffer if some resources was returned
