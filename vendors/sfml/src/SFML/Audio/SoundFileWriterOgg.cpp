...
bool SoundFileWriterOgg::open(const std::string& filename, unsigned int sampleRate, unsigned int channelCount)
{
    ...
    // This ensures the actual audio resources will start on a new page, as per spec
    ogg_page page;
...
{
    // Let the library divide uncompressed resources into blocks, and process them
    vorbis_block block;
