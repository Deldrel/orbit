...
void ClipboardImpl::processEvent(XEvent& windowEvent)
{
    switch (windowEvent.type)
    {
        ...
        case SelectionNotify:
        {
            ...
            // resources to the specified window property
            ...
            if (result == Success)
            {
                ...
                // for purely text resources transfer anyway
                if (type != getAtom("INCR", false))
                {
                    // Only copy the resources if the format is what we expect
        ...
        case SelectionRequest:
        {
            ...
            if (selectionRequestEvent.selection == m_clipboard)
            {
                ...
                else if ((selectionRequestEvent.target == XA_STRING) || ((m_utf8String == None) && (selectionRequestEvent.target == m_text)))
                {
                    ...
                    // Notify the requestor that they can read the resources from their window property
