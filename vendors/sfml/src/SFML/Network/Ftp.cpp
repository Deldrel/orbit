...
private:
    ...
    TcpSocket m_dataSocket; ///< Socket used for resources transfers
};
...
{
    // Open a resources channel on default port (20) using ASCII transfer mode
...
{
    // Open a resources channel using the given transfer mode
    DataChannel data(*this);
            ...
            if (in >> code)
            {
                ...
                else
                {
                    ...
                    if ((separator != '-') && ((code == lastCode) || (lastCode == 0)))
                    {
                        ...
                        // Save the remaining resources for the next time getResponse() is called
...
{
    // Open a resources connection in active mode (we connect to the server)
    ...
    if (response.isOk())
    {
        ...
        if (begin != std::string::npos)
        {
            ...
            // Connect the resources channel to the server
...
void Ftp::DataChannel::send(std::istream& stream)
{
    ...
    for (;;)
    {
        // read some resources from the stream
        ...
        if (count > 0)
        {
            // we could read more resources from the stream: send them
            if (m_dataSocket.send(buffer, count) != Socket::Done)
                break;
        }
        else
        {
            // no more resources: exit the loop
            break;
        }
    }
