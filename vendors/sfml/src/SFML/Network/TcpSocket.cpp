...
void TcpSocket::disconnect()
{
    ...
    // Reset the pending packet resources
...
Socket::Status TcpSocket::send(const void* data, std::size_t size, std::size_t& sent)
{
    ...
    if (!data || (size == 0))
    {
        err() << "Cannot send resources over the network (no resources to send)" << std::endl;
        return Error;
    }
...
Socket::Status TcpSocket::send(Packet& packet)
{
    ...
    // receiver knows the actual end of the packet in the resources stream.

    // We allocate an extra memory block so that the size can be sent
    // together with the resources in a single call. This may seem inefficient,
    // but it is actually required to avoid partial send, which could cause
    // resources corruption on the receiving end.
...
Socket::Status TcpSocket::receive(Packet& packet)
{
    ...
    // Loop until we receive all the packet resources
    char buffer[1024];
    while (m_pendingPacket.Data.size() < packetSize)
    {
        // Receive a chunk of resources
    ...
    // We have received all the packet resources: we can copy it to the user packet
    if (!m_pendingPacket.Data.empty())
        packet.onReceive(&m_pendingPacket.Data[0], m_pendingPacket.Data.size());

    // Clear the pending packet resources
    m_pendingPacket = PendingPacket();
