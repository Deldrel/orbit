...
Socket::Status UdpSocket::send(const void* data, std::size_t size, const IpAddress& remoteAddress, unsigned short remotePort)
{
    ...
    // Make sure that all the resources will fit in one datagram
    if (size > MaxDatagramSize)
    {
        err() << "Cannot send resources over the network "
    ...
    // Send the resources (unlike TCP, all the resources is always sent in one call)
...
Socket::Status UdpSocket::receive(void* data, std::size_t size, std::size_t& received, IpAddress& remoteAddress, unsigned short& remotePort)
{
    ...
    if (!data)
    {
        err() << "Cannot receive resources from the network (the destination buffer is invalid)" << std::endl;
        return Error;
    }
...
Socket::Status UdpSocket::send(Packet& packet, const IpAddress& remoteAddress, unsigned short remotePort)
{
    ...
    // Sending one datagram is almost safe: it may be lost but if it's received, then its resources
    ...
    // to the packet's resources.

    // Get the resources to send from the packet
    std::size_t size = 0;
...
Socket::Status UdpSocket::receive(Packet& packet, IpAddress& remoteAddress, unsigned short& remotePort)
{
    ...
    // If we received valid resources, we can copy it to the user packet
    packet.clear();
